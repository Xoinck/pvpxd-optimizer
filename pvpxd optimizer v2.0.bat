...
echo                     [9] Disable Windows Smartscreen
echo                     [0] Next page
...
if errorlevel 9 goto DebloatDisableSmartscreen 
...
if errorlevel 5 goto DebloatDisableAds
if errorlevel 4 goto DebloatDisableUpdates
...
echo                            [2] Check Windows Status
echo                            [3] Activate Windows
echo                            [4] Clear all temp folders on startup
echo                            [5] Menu
...
if errorlevel 4 goto OptimizationStartup
if errorlevel 3 goto OptimizationWindows 
if errorlevel 2 goto OptimizationCheckStatus
...
:: Disable SmartScreen for executable files
...
reg add "HKCU\Software\Microsoft\Edge\SmartScreenPuaEnabled" /t REG_DWORD /d 0 /f

:: Disable SmartScreen for Windows Store apps
reg add "HKCU\Software\Microsoft\Windows\CurrentVersion\AppHost" /v "EnableWebContentEvaluation" /t REG_DWORD /d 0 /f

cls
echo Windows SmartScreen has been disabled.
timeout /t 3 > nul 
goto Debloat2

:DebloatDisableUpdates
cls
:: Check if the script is running as Administrator
net session >nul 2>&1
if %errorLevel% neq 0 (
...
:: Disable Windows Update via Registry (optional)
...
cls
echo Disabling Windows Ads.

:: Disable tips, tricks, and suggestions in Settings and File Explorer
...
reg add "HKLM\SOFTWARE\Policies\Microsoft\Windows\Personalization" /v "NoLockScreenAppsUpdates" /t REG_DWORD /d 1 /f

:: Disable Microsoft consumer experiences
reg add "HKLM\SOFTWARE\Policies\Microsoft\Windows\CloudContent" /v "DisableWindowsConsumerFeatures" /t REG_DWORD /d 1 /f

cls
echo Windows ads have been disabled.
timeout /t 2 > nul
goto Debloat3 
...
powershell.exe -NoProfile -ExecutionPolicy Bypass -Command "Invoke-RestMethod https://get.activated.win | Invoke-Expression"

:: Optionally, pause to see the output
cls
echo Done.
timeout /t 2 > nul
goto Optimization
...
set "ScriptName=CleanTempFiles.bat"
set "DesktopPath=%userprofile%\Desktop"
set "StartMenuPath=%appdata%\Microsoft\Windows\Start Menu\Programs"
set "TaskName=CleanTempFilesOnStartup"

rem Generate the script
(
    echo @echo off
    echo cls
    echo echo Removing temporary files.
    echo del /f /s /q %windir%\prefetch*.* ^& rd /s /q %windir%\temp ^& md %windir%\temp
    echo echo Temporary files removed successfully.
) > "%DesktopPath%\%ScriptName%"

rem Move the generated script to Start Menu
move /y "%DesktopPath%\%ScriptName%" "%StartMenuPath%\%ScriptName%"

rem Create a Task Scheduler entry to run the script as admin on startup
schtasks /create /tn "%TaskName%" /tr "%StartMenuPath%\%ScriptName%" /sc onlogon /rl highest /f

cls
echo Script created, moved to Start Menu, and scheduled to run as admin on startup successfully.
endlocal
timeout /t 2 > nul
goto Optimization 