...
REM Get the username of the currently logged in user
set "tempUserFolder=%USERPROFILE%\AppData\Local\Temp"

echo                  Clearing temporary folders
REM Deleting all files and folders in the user's temp directory
del /q /f /s "%tempUserFolder%\*.*"

for /d %%x in ("%tempUserFolder%\*") do rmdir /s /q "%%x"

REM Deleting all files and folders in the system temp directory
set "systemTempFolder=C:\Windows\Temp"
del /q /f /s "%systemTempFolder%\*.*"
for /d %%x in ("%systemTempFolder%\*") do rmdir /s /q "%%x"

echo                        Temp files have been cleared successfully.
timeout /t 2 > nul 
goto Debloat 
